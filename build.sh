...
echo "🚀 Building PikaLink for production..."
...
go mod tidy
go build -o ../dist/pikalink main.go
if [ $? -ne 0 ]; then
...
cp backend/pikalink.db dist/ 2>/dev/null || echo "No existing database found, will be created on first run"
...
echo -e "${GREEN}🚀 To run: cd dist && ./pikalink${NC}"
