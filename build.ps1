...
go mod tidy
go build -o "../dist/pikalink.exe" main.go
if ($LASTEXITCODE -ne 0) {
...
if (Test-Path "backend/pikalink.db") {
    Copy-Item "backend/pikalink.db" "dist/"
} else {
...
Write-Host "To run: cd dist; ./pikalink.exe" -ForegroundColor Green